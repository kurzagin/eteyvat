...
export default async function BannerRotationPage() {
  ...
  const charactersByPhase = new Map<number, Array<{ slug: string; name: string; rarity: number }>>();
  ...
  if (phaseIds.size) {
    ...
    for (const character of allCharacters) {
      ...
      phaseCharacters.push({ slug: character.slug, name: character.name, rarity: character.rarity });
  ...
  return (
    <div className="banner-subpage">
      <header className="banner-subpage-hero">
        <Link className="banner-back-link" href="/banners"><ArrowLeft size={13} /> Observatory</Link>
                  ...
                  <div className="phase-five-stars">
                    {fiveStars.map((character) => (
                      <Link href={`/characters/${character.slug}/banner-history`} key={character.slug}>
                        <CharacterPortrait slug={character.slug} name={character.name} sizes="130px" />
